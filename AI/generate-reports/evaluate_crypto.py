...
def analyze_white_paper(crypto_name):
    query = '''
You are an AI that evaluates the investment potential of cryptocurrencies. Your goal is to provide a score out of 10 based on the provided market metrics. Use the following criteria to assign the score:
...
def analyze_sentiment(crypto_name, crypto_ticker, api_key):
    ...
    for article in articles:
        # Assuming full article text is available in 'content', if not present, use title and description
        try:
        ...
        # Filter text to include only sentences mentioning the cryptocurrency
        filtered_text = filter_relevant_sentences(text, crypto_name, relevant_keywords)
        # Perform sentiment analysis in chunks
        chunk_scores = []
        ...
        # Aggregate chunk scores by averaging
        if chunk_scores:
...
def fetch_market_metrics(crypto_id, crypto_ticker):
    ...
    if response.status_code == 200:
        market_data = response.json()['data'][crypto_id] 
    else:
...
def evaluate_crypto(crypto_id, crypto_name, crypto_ticker):
    website_check, site_score = check_website(crypto_name) # 5
    
    white_paper_check, paper_score = analyze_white_paper(crypto_name) # 15

    market_metrics = fetch_market_metrics(crypto_id, crypto_ticker) 
    market_metrics_check, market_score = evaluate_metrics(market_metrics) # 50

    sentiment_df, average_sentiment, sentiment_sum = analyze_sentiment(crypto_name, crypto_ticker, NEWSAPI_API_KEY) # 15
    sentiment_check, sentiment_score = evaluate_sentiment(sentiment_df, average_sentiment, sentiment_sum)

    utility_check,util_score = check_utility(crypto_name) # 15
...
if __name__ == "__main__":
    ...
    for i in range(5):
        try:
            evaluate_crypto(crypto_id, crypto_name, crypto_ticker)
    ...
    '''sentiment_df, average_sentiment, sentiment_sum = analyze_sentiment(crypto_name, crypto_ticker, NEWSAPI_API_KEY) # 15
    sentiment_check, sentiment_score = evaluate_sentiment(sentiment_df, average_sentiment, sentiment_sum)
    troubleshoot(sentiment_check)'''



    '''tavily = TavilyClient(api_key=TAVILY_API_KEY)
    response = tavily.get_search_context(query=f"What is the content in the white paper for {crypto_name}?", search_depth="advanced")'''
    #print(response) #response['url'], response['title'], response['content'], response['raw_content']
    #main("BTC-USD", "Bitcoin", "https://bitcoin.org", "https://bitcoin.org/bitcoin.pdf")

# ADD APACHE SPARK TO OPTIMIZE RUN TIME
